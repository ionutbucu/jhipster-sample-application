...
/**
 * Performance test for the ReportExecution entity.
 */
public class ReportExecutionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all reportExecutions").get("/api/report-executions").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new reportExecution")
                        .post("/api/report-executions")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_reportExecution_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created reportExecution").get("${new_reportExecution_url}").headers(headers_http_authenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created reportExecution").delete("${new_reportExecution_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ReportExecution entity").exec(scn);
