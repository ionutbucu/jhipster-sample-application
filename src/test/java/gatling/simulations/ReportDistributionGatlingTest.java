...
/**
 * Performance test for the ReportDistribution entity.
 */
public class ReportDistributionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all reportDistributions")
                ...
                .exec(
                    http("Create new reportDistribution")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_reportDistribution_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created reportDistribution").get("${new_reportDistribution_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(http("Delete created reportDistribution").delete("${new_reportDistribution_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ReportDistribution entity").exec(scn);
