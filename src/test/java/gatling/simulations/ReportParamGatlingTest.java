...
/**
 * Performance test for the ReportParam entity.
 */
public class ReportParamGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all reportParams").get("/api/report-params").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new reportParam")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"type\": \"SAMPLE_TEXT\"" +
                                ", \"value\": \"SAMPLE_TEXT\"" +
                                ", \"conversionRule\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_reportParam_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created reportParam").get("${new_reportParam_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created reportParam").delete("${new_reportParam_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ReportParam entity").exec(scn);
