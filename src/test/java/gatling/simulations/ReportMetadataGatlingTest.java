...
/**
 * Performance test for the ReportMetadata entity.
 */
public class ReportMetadataGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all reportMetadata").get("/api/report-metadata").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new reportMetadata")
                        .post("/api/report-metadata")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"rid\": \"SAMPLE_TEXT\"" + ", \"metadata\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_reportMetadata_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created reportMetadata").get("${new_reportMetadata_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created reportMetadata").delete("${new_reportMetadata_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ReportMetadata entity").exec(scn);
