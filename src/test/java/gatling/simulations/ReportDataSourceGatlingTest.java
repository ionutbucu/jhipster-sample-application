...
/**
 * Performance test for the ReportDataSource entity.
 */
public class ReportDataSourceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all reportDataSources")
                ...
                .exec(
                    http("Create new reportDataSource")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_reportDataSource_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created reportDataSource").get("${new_reportDataSource_url}").headers(headers_http_authenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created reportDataSource").delete("${new_reportDataSource_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ReportDataSource entity").exec(scn);
