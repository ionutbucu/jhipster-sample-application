...
/**
 * Performance test for the ReportSchedule entity.
 */
public class ReportScheduleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all reportSchedules").get("/api/report-schedules").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new reportSchedule")
                        .post("/api/report-schedules")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"rid\": \"SAMPLE_TEXT\"" + ", \"cron\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_reportSchedule_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created reportSchedule").get("${new_reportSchedule_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created reportSchedule").delete("${new_reportSchedule_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ReportSchedule entity").exec(scn);
