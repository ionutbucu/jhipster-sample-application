...
/**
 * Performance test for the ReportColumnMapping entity.
 */
public class ReportColumnMappingGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all reportColumnMappings")
                    .get("/api/report-column-mappings")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new reportColumnMapping")
                        .post("/api/report-column-mappings")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"rid\": \"SAMPLE_TEXT\"" +
                                ", \"sourceColumnName\": \"SAMPLE_TEXT\"" +
                                ", \"sourceColumnIndex\": 0" +
                                ", \"columnTitle\": \"SAMPLE_TEXT\"" +
                                ", \"lang\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_reportColumnMapping_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created reportColumnMapping").get("${new_reportColumnMapping_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created reportColumnMapping").delete("${new_reportColumnMapping_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ReportColumnMapping entity").exec(scn);
